...
describe('WalletService', () => {
  ...
  describe('getOrCreateWallet', () => {
    it('should return an existing wallet', async () => {
      ...
      (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(mockWallet);
      ...
      expect(prisma.wallets.findUnique).toHaveBeenCalledWith({
    ...
    it('should create a new wallet with a welcome bonus if one does not exist', async () => {
        ...
        (prisma.users.findUnique as jest.Mock).mockResolvedValue(mockUser);
        (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(null);
        (prisma.wallets.create as jest.Mock).mockResolvedValue(newWallet);
        (prisma.wallet_transactions.create as jest.Mock).mockResolvedValue({});
  
        const result = await service.getOrCreateWallet(mockUser.id);
  
        expect(prisma.wallets.create).toHaveBeenCalledWith({
  ...
  describe('addFunds', () => {
    it('should add only MXN funds to an existing wallet', async () => {
      ...
      (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(mockWallet);
      (prisma.wallets.update as jest.Mock).mockResolvedValue(updatedWalletData);

      const result = await service.addFunds(mockUser.id, 100, 0, 'Test Deposit');

      expect(prisma.wallets.findUnique).toHaveBeenCalledWith({ where: { userId: mockUser.id } });
      ...
      expect(prisma.wallet_transactions.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should create a wallet if it does not exist and then add funds', async () => {
        ...
        (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(null);
        (prisma.wallets.create as jest.Mock).mockResolvedValue(newWallet);
        (prisma.wallets.update as jest.Mock).mockResolvedValue(newWallet); // This call is now important
  ...
  describe('transferFunds', () => {
    it('should transfer funds successfully to a user with a wallet', async () => {
      (prisma.users.findUnique as jest.Mock).mockResolvedValue(mockRecipientUser);
      (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(mockWallet);
      (prisma.wallets.update as jest.Mock).mockResolvedValue(mockWallet); 

      const result = await service.transferFunds(mockUser.id, mockRecipientUser.email, 100, 50);

      expect(prisma.users.findUnique).toHaveBeenCalledWith({ where: { email: mockRecipientUser.email }, include: { wallet: true } });
      expect(prisma.wallets.findUnique).toHaveBeenCalledWith({ where: { userId: mockUser.id } });
      expect(prisma.$transaction).toHaveBeenCalled();
      expect(prisma.wallets.update).toHaveBeenCalledTimes(2);
      expect(prisma.wallet_transactions.create).toHaveBeenCalledTimes(2);
    ...
    it('should create a wallet for the recipient if it does not exist during transfer', async () => {
        ...
        (prisma.users.findUnique as jest.Mock).mockResolvedValue(recipientWithoutWallet);
        (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(mockWallet);
        (prisma.wallets.create as jest.Mock).mockResolvedValue(newRecipientWallet);
        (prisma.wallets.update as jest.Mock).mockImplementation((args) => {
    ...
    it('should throw HttpException if sender wallet is not found', async () => {
      ...
      it('should throw HttpException for insufficient MXN funds', async () => {
        ...
        (prisma.users.findUnique as jest.Mock).mockResolvedValue(mockRecipientUser);
        (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(poorWallet);
        await expect(service.transferFunds(mockUser.id, mockRecipientUser.email, 100, 0)).rejects.toThrow(
  ...
  describe('getTransactions', () => {
    it('should return paginated transactions for a wallet', async () => {
      ...
      (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(mockWallet);
      (prisma.wallet_transactions.findMany as jest.Mock).mockResolvedValue(mockTransactions);
      (prisma.wallet_transactions.count as jest.Mock).mockResolvedValue(5);

      const result = await service.getTransactions(mockUser.id, 2, 0);

      expect(prisma.wallets.findUnique).toHaveBeenCalledWith({ where: { userId: mockUser.id } });
      expect(prisma.wallet_transactions.findMany).toHaveBeenCalledWith({
        where: { walletId: mockWallet.id },
        orderBy: { createdAt: 'desc' },
        take: 2,
        skip: 0,
      });
      expect(prisma.wallet_transactions.count).toHaveBeenCalledWith({ where: { walletId: mockWallet.id } });
  ...
  describe('convertCurrency', () => {
    it('should convert MXN to AXO successfully and create a transaction', async () => {
      (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(mockWallet);
      (prisma.wallets.update as jest.Mock).mockResolvedValue({ ...mockWallet, balanceMXN: 900, balanceAxo: 580 });

      const result = await service.convertCurrency(mockUser.id, 'MXN', 'AXO', 100);

      expect(prisma.wallets.update).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should convert AXO to MXN successfully and create a transaction', async () => {
        (prisma.wallets.findUnique as jest.Mock).mockResolvedValue(mockWallet);
        (prisma.wallets.update as jest.Mock).mockResolvedValue({ ...mockWallet, balanceMXN: 1110, balanceAxo: 400 });
  
        const result = await service.convertCurrency(mockUser.id, 'AXO', 'MXN', 100);
  
        const updateCall = (prisma.wallets.update as jest.Mock).mock.calls[0][0];
        expect(updateCall.data.balanceAxo.increment).toBe(-100);
        expect(updateCall.data.balanceMXN.increment).toBeCloseTo(110);
                const transactionCall = (prisma.wallet_transactions.create as jest.Mock).mock.calls[0][0];
