...
describe('WalletService', () => {
    ...
    describe('getOrCreateWallet', () => {
        it('should return an existing wallet', async () => {
            ...
            prisma.wallets.findUnique.mockResolvedValue(mockWallet);
        ...
        it('should create a new wallet with a welcome bonus if one does not exist', async () => {
            ...
            prisma.wallets.create.mockResolvedValue(newWallet);
            prisma.wallet_transactions.create.mockResolvedValue({});
            const result = await service.getOrCreateWallet(mockUser.id);
    ...
    describe('addFunds', () => {
        it('should add only MXN funds to an existing wallet', async () => {
            ...
            prisma.wallets.findUnique.mockResolvedValue(mockWallet);
            prisma.wallets.update.mockResolvedValue(updatedWalletData);
        ...
        it('should create a wallet if it does not exist and then add funds', async () => {
            ...
            prisma.wallets.findUnique.mockResolvedValue(null);
            prisma.wallets.create.mockResolvedValue(newWallet);
            prisma.wallets.update.mockResolvedValue(newWallet);
    ...
    describe('transferFunds', () => {
        it('should transfer funds successfully to a user with a wallet', async () => {
            ...
            prisma.wallets.findUnique.mockResolvedValue(mockWallet);
            prisma.wallets.update.mockResolvedValue(mockWallet);
        ...
        it('should create a wallet for the recipient if it does not exist during transfer', async () => {
            const recipientWithoutWallet = Object.assign(Object.assign({}, mockRecipientUser), { wallet: null });
            ...
            prisma.wallets.findUnique.mockResolvedValue(mockWallet);
            prisma.wallets.create.mockResolvedValue(newRecipientWallet);
            prisma.wallets.update.mockImplementation((args) => {
        ...
        it('should throw HttpException for insufficient MXN funds', async () => {
            ...
            prisma.users.findUnique.mockResolvedValue(mockRecipientUser);
            prisma.wallets.findUnique.mockResolvedValue(poorWallet);
    ...
    describe('getTransactions', () => {
        it('should return paginated transactions for a wallet', async () => {
            ...
            prisma.wallets.findUnique.mockResolvedValue(mockWallet);
            prisma.wallet_transactions.findMany.mockResolvedValue(mockTransactions);
            prisma.wallet_transactions.count.mockResolvedValue(5);
    ...
    describe('convertCurrency', () => {
        it('should convert MXN to AXO successfully and create a transaction', async () => {
            prisma.wallets.findUnique.mockResolvedValue(mockWallet);
            prisma.wallets.update.mockResolvedValue(Object.assign(Object.assign({}, mockWallet), { balanceMXN: 900, balanceAxo: 580 }));
        ...
        it('should convert AXO to MXN successfully and create a transaction', async () => {
            prisma.wallets.findUnique.mockResolvedValue(mockWallet);
            prisma.wallets.update.mockResolvedValue(Object.assign(Object.assign({}, mockWallet), { balanceMXN: 1110, balanceAxo: 400 }));
            const result = await service.convertCurrency(mockUser.id, 'AXO', 'MXN', 100);
            const updateCall = prisma.wallets.update.mock.calls[0][0];
            expect(updateCall.data.balanceAxo.increment).toBe(-100);
            expect(updateCall.data.balanceMXN.increment).toBeCloseTo(110);
            const transactionCall = prisma.wallet_transactions.create.mock.calls[0][0];
...
describe('WalletService', () => {
    ...
    describe('transferFunds', () => {
        it('should transfer funds successfully to a user with a wallet', async () => {
            prisma.user.findUnique.mockResolvedValue(mockRecipientUser);
            prisma.wallet.findUnique.mockResolvedValue(mockWallet);
        ...
        it('should create a wallet for the recipient if it does not exist during transfer', async () => {
            const recipientWithoutWallet = Object.assign(Object.assign({}, mockRecipientUser), { wallet: null });
            const newRecipientWallet = { id: 'new-wallet-2', userId: recipientWithoutWallet.id, balanceMXN: 0, balanceAxo: 50 };
            prisma.user.findUnique.mockResolvedValue(recipientWithoutWallet);
            prisma.wallet.findUnique.mockResolvedValue(mockWallet);
